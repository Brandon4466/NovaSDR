...
type PersistedAudioSettings = Pick<
  AudioUiSettings,
  'agcSpeed' | 'agcAttackMs' | 'agcReleaseMs' | 'bufferMode' | 'deemphasis'
>;
...
function readPersistedAudioSettings(fallback: PersistedAudioSettings): PersistedAudioSettings {
  ...
  if (raw.deemphasis === 'off' || raw.deemphasis === '75' || raw.deemphasis === '50') out.deemphasis = raw.deemphasis;
  return out;
}
...
export default function App() {
  ...
  const [audioSettings, setAudioSettings] = useState<AudioUiSettings>(() => {
    ...
    const persisted = readPersistedAudioSettings({
      ...
      deemphasis: fallback.deemphasis,
    });
  ...
  useEffect(() => {
    ...
    try {
      const persisted: PersistedAudioSettings = {
        ...
        deemphasis: audioSettings.deemphasis,
      };
