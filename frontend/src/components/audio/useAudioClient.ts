...
export function useAudioClient({ receiverId, receiverSessionNonce, mode, centerHz, settings, audioWindow, forceRetuneHz, onPcm }: Props) {
  ...
  const lastSentDeemphasisRef = useRef<string | null>(null);
  const lastSentTuneRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    lastSentDeemphasisRef.current = null;
  ...
  useEffect(() => {
    if (!basicInfo) return;
    const mode = settings.deemphasis;
    if (lastSentDeemphasisRef.current === mode) return;
    if (!send({ cmd: 'deemphasis', mode })) return;
    lastSentDeemphasisRef.current = mode;
  }, [basicInfo, connectionNonce, send, settings.deemphasis]);
