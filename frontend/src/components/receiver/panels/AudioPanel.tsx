...
import type { AudioDebugStats, AudioUiSettings, DeEmphasisMode } from '../../audio/types';
        ...
        {isFmMode(mode) ? (
          <div className="space-y-2">
            <div className="flex items-center gap-1.5">
              <Label>De-emphasis</Label>
              <HelpTip
                title="FM De-emphasis"
                text="Restores flat frequency response on FM broadcast audio. Use 75µs in the Americas/Korea, 50µs in Europe and most of the rest of the world."
              />
            </div>
            <div className="grid grid-cols-3 gap-2">
              {(['off', '75', '50'] as DeEmphasisMode[]).map((opt) => (
                <Button
                  key={opt}
                  type="button"
                  size="sm"
                  variant={settings.deemphasis === opt ? 'default' : 'secondary'}
                  onClick={() => onChange((prev) => ({ ...prev, deemphasis: opt }))}
                >
                  {opt === 'off' ? 'Off' : `${opt}µs`}
                </Button>
              ))}
...
function isFmMode(mode: ReceiverMode): boolean {
  return mode === 'FM' || mode === 'FMC' || mode === 'WBFM';
}
